...
const EditPost = () => {
  const bodyRef = useRef(data.posts[0].body);
  const editorRef = useRef(null);
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [file, setFile] = useState(data.posts[0].file || null);
  const titleRef = useRef(data.posts[0].title);
